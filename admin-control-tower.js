(()=>{
 ...
 function render(data){const s=data.summary||{};metric('towerActiveStores',s.activeStores);metric('towerInteractions',s.totalRecent);metric('towerDirections',s.directions);metric('towerBenefits',s.benefits);metric('towerRising',s.rising);metric('towerWatch',s.watch);metric('towerExperiments',s.experiments);metric('towerCanonicalStores',data.identity?.canonicalStores);const period=$('towerPeriod');if(period)period.textContent=`${data.recentFrom||''} ~ ${data.to||''}`;
  ...
  const status=$('towerStatus');if(status)status.textContent=`최근 반응이 발생한 ${Number(s.activeStores||0).toLocaleString('ko-KR')}개 점포를 익명 집계로 살펴봤습니다. 순위가 아니라 운영 개입 후보만 표시합니다. 점포명 변경은 연결된 점포 ID 기준으로 같은 이력에 이어집니다.`;
 }
