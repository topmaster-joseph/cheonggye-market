...
assert.equal([...home.matchAll(/id="resources"/g)].length,1);assert.doesNotMatch(home,/id="mapListToggle"/);assert.match(home,/class="map-center"/);assert.match(home,/id="resourceSearch"/);assert.ok(home.includes('cgma-sections.css?v=20260905-benefit-map-v6'));assert.doesNotMatch(home,/leaflet@|openstreetmap/i);
...
const redesign=await readFile('cgma-sections.css','utf8');assert.match(redesign,/market-pin/);assert.match(redesign,/map-center/);assert.match(redesign,/muan-map-art/);assert.match(redesign,/cartoon-pin/);assert.match(redesign,/resource-filterbar/);const mapScript=await readFile('script.js','utf8');assert.match(mapScript,/mapIllustration/);assert.match(mapScript,/muan-boundary/);assert.match(mapScript,/stick-person/);assert.doesNotMatch(mapScript,/L\.map|tile\.openstreetmap|FIXED_MAP_ZOOM/);assert.match(mapScript,/mapDirectoryCount/);assert.match(mapScript,/renderTodayDiscovery/);assert.match(mapScript,/revealRecommendedShop/);assert.match(mapScript,/benefitOnlyToggle/);assert.match(mapScript,/benefit-pin-badge/);assert.match(mapScript,/activeBenefitEntries/);assert.match(home,/id="todayDiscovery"/);assert.match(home,/id="todayPickRefresh"/);assert.match(home,/id="benefitOnlyToggle"/);const archiveScript=await readFile('resources.js','utf8');assert.match(archiveScript,/data-resource-filter/);assert.match(archiveScript,/resourceSearch/);
