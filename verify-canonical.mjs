...
assert.equal([...home.matchAll(/id="resources"/g)].length,1);assert.doesNotMatch(home,/id="mapListToggle"/);assert.match(home,/class="map-center"/);assert.match(home,/id="resourceSearch"/);assert.ok(home.includes('cgma-sections.css?v=20260904-fixed-map'));
...
const redesign=await readFile('cgma-sections.css','utf8');assert.match(redesign,/market-pin/);assert.match(redesign,/map-center/);assert.match(redesign,/resource-filterbar/);const mapScript=await readFile('script.js','utf8');assert.match(mapScript,/data-fixed-map|fixedMap/);assert.match(mapScript,/dragging:false/);assert.match(mapScript,/zoomControl:false/);assert.match(mapScript,/mapDirectoryCount/);const archiveScript=await readFile('resources.js','utf8');assert.match(archiveScript,/data-resource-filter/);assert.match(archiveScript,/resourceSearch/);
