...
const SITE='cgma';
const AUTH_REDIRECT='https://cgma.ekodi.kr/member?apply=1';
...
async function loadClaims(){const d=await api(MEMBERSHIP,'/mine');const claims=d.claims||[];if(d.profile){$('displayName').value=d.profile.display_name||'';$('phone').value=d.profile.phone||'';}show('claimsCard',true);const label={pending:'점포 연결 대기',approved:'점포 연결 완료',rejected:'반려',withdrawn:'철회'};$('claims').innerHTML=claims.length?claims.map(c=>`<div class="claim"><div class="row claim-head"><b>${esc(c.store?.name||'점포')}</b><span class="badge ${esc(c.status)}">${label[c.status]||esc(c.status)}</span></div><p class="subtle">${esc(c.store?.slug||'')} · ${new Date(c.requested_at).toLocaleDateString('ko-KR')}</p>${c.admin_note?`<p class="subtle">확인 메모: ${esc(c.admin_note)}</p>`:''}${c.status==='approved'?`<button class="ghost connect" data-store="${esc(c.store_id)}">이 가게 AI 연결</button>`:''}${c.status==='pending'?`<button class="secondary withdraw" data-id="${esc(c.id)}">신청 철회</button>`:''}</div>`).join(''):'<div class="status">정회원 승인이 완료되었습니다. 아래에서 내 가게를 선택해 연결해 주세요.</div>';$('claims').querySelectorAll('.connect').forEach(b=>b.onclick=()=>{activeApprovedStore=b.dataset.store;show('serviceCard',true);setStage(4);$('serviceCard').scrollIntoView({behavior:'smooth',block:'start'});hideStatus('serviceStatus');});$('claims').querySelectorAll('.withdraw').forEach(b=>b.onclick=async()=>{try{await api(MEMBERSHIP,'/withdraw',{method:'POST',body:JSON.stringify({claim_id:b.dataset.id})});await loadClaims()}catch{status('claimStatus','철회 처리에 실패했습니다.','error')}});const firstApproved=claims.find(c=>c.status==='approved');if(firstApproved&&!activeApprovedStore)activeApprovedStore=firstApproved.store_id;if(firstApproved)show('serviceCard',true);setStage(4);}
