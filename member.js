...
const {data:{session:initial}}=await sb.auth.getSession();if(initial){history.replaceState({},document.title,location.pathname);await signedInUI(initial)}else await signedOutUI();
sb.auth.onAuthStateChange(async(event,s)=>{if(event==='SIGNED_IN'&&s){history.replaceState({},document.title,location.pathname);await signedInUI(s)}if(event==='SIGNED_OUT')await signedOutUI()});
