...
function setStage(stage){document.querySelectorAll('.progress-item').forEach(item=>{const n=Number(item.dataset.stage);item.classList.toggle('active',n===stage);item.classList.toggle('done',n<stage);item.setAttribute('aria-current',n===stage?'step':'false')});const label={1:'이메일 로그인',2:'내 가게 선택',3:'승인 확인',4:'서비스 연결'};const current=$('progressCurrent');if(current)current.textContent=`현재 ${stage}단계 · ${label[stage]}`;}
...
async function oauth(provider){status('loginStatus',`${provider==='google'?'Google':'Kakao'} 로그인 화면으로 이동합니다.`);const {error}=await sb.auth.signInWithOAuth({provider,options:{redirectTo:AUTH_REDIRECT}});if(error)status('loginStatus','로그인 제공자 또는 Redirect 설정을 확인해 주세요.','error');}
...
async function signedInUI(s){show('loginCard',false);show('accountCard',true);show('storeCard',true);$('accountEmail').textContent=s.user.email||s.user.user_metadata?.full_name||'정회원 계정';setStage(2);try{await Promise.all([loadStores(),loadClaims()])}catch{status('claimStatus','회원 정보를 불러오지 못했습니다. 잠시 후 다시 시도해 주세요.','error')}}
async function signedOutUI(){show('loginCard',true);show('accountCard',false);show('storeCard',false);show('claimsCard',false);show('serviceCard',false);setStage(1)}

$('googleLogin').onclick=()=>oauth('google');
$('kakaoLogin').onclick=()=>oauth('kakao');
$('sendLink').onclick=async()=>{const email=$('email').value.trim();if(!email)return status('loginStatus','이메일을 입력해 주세요.','warn');$('sendLink').disabled=true;try{const {error}=await sb.auth.signInWithOtp({email,options:{emailRedirectTo:AUTH_REDIRECT,shouldCreateUser:true}});if(error)throw error;status('loginStatus','로그인 링크를 보냈습니다. 가장 최근 메일의 링크를 한 번만 열어 주세요.')}catch{status('loginStatus','로그인 링크를 보내지 못했습니다. 잠시 후 다시 시도해 주세요.','error')}finally{$('sendLink').disabled=false}};
