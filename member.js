...
$('sendLink').onclick=async()=>{const email=$('email').value.trim();if(!email)return status('loginStatus','이메일을 입력해 주세요.','warn');$('sendLink').disabled=true;try{const {error}=await sb.auth.signInWithOtp({email,options:{emailRedirectTo:`${location.origin}/member`,shouldCreateUser:true}});if(error)throw error;status('loginStatus','로그인 링크를 보냈습니다. 이메일에서 링크를 열어 주세요.')}catch{status('loginStatus','로그인 링크를 보내지 못했습니다. 잠시 후 다시 시도해 주세요.','error')}finally{$('sendLink').disabled=false}};
