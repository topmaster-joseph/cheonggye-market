...
async function signedInUI(s){show('loginCard',false);show('accountCard',true);show('storeCard',true);$('accountEmail').textContent=s.user.email||'정회원 계정';try{await Promise.all([loadStores(),loadClaims()])}catch{status('claimStatus','회원 정보를 불러오지 못했습니다. 잠시 후 다시 시도해 주세요.','error')}}
...
$('sendLink').onclick=async()=>{const email=$('email').value.trim();if(!email)return status('loginStatus','이메일을 입력해 주세요.','warn');$('sendLink').disabled=true;try{const redirect=`${location.origin}/member`;const {error}=await sb.auth.signInWithOtp({email,options:{emailRedirectTo:redirect,shouldCreateUser:true}});if(error)throw error;status('loginStatus','로그인 링크를 보냈습니다. 이메일에서 링크를 열어 주세요.')}catch{status('loginStatus','로그인 링크를 보내지 못했습니다. 운영 도메인/Auth 설정을 확인해 주세요.','error')}finally{$('sendLink').disabled=false}};
...
$('activateService').onclick=async()=>{if(!activeApprovedStore)return status('serviceStatus','승인된 점포를 먼저 선택해 주세요.','warn');$('activateService').disabled=true;try{status('serviceStatus','공개 점포정보를 안전하게 연결하고 있습니다.');const d=await api(ONBOARDING,'/activate',{method:'POST',body:JSON.stringify({source_store_id:activeApprovedStore,target_tenant:'ekodibiz',copy_public_menu:$('copyMenu').checked,consent:true})});try{await api(KNOWLEDGE,'/bootstrap',{method:'POST',body:JSON.stringify({store_id:d.target.store_id})})}catch{}status('serviceStatus','EKODIBIZ 서비스 연결이 완료되었습니다. 기본 점포정보도 AI 지식으로 준비했습니다.')}catch(e){status('serviceStatus',e.message==='merchant_access_required'?'점포 승인 권한을 확인해 주세요.':'서비스 연결을 완료하지 못했습니다.','error')}finally{$('activateService').disabled=false}};
