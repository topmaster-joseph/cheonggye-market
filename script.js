...
let memberFilter='all',categoryFilter='all',benefitOnly=false;
...
async function loadPublicStoreProfiles(){const selected=activeShopIndex;try{const r=await fetch(PUBLIC_PROFILE_URL,{headers:{apikey:PUBLIC_PROFILE_KEY,Authorization:`Bearer ${PUBLIC_PROFILE_KEY}`}});if(!r.ok)throw new Error('profile_http');publicStoreProfiles=await r.json();renderMap();renderTodayDiscovery();if(selected!==null&&(!benefitOnly||hasActiveBenefit(shops[selected])))selectShop(selected);}catch{publicStoreProfiles=[];renderMap();renderTodayDiscovery();if(selected!==null&&!benefitOnly)selectShop(selected);}}
...
function revealRecommendedShop(index){if(index<0||!shops[index])return;if(mapSearch)mapSearch.value='';memberFilter='all';categoryFilter='all';benefitOnly=false;document.querySelectorAll('[data-member-filter],[data-category-filter]').forEach(button=>button.classList.toggle('active',button.dataset.memberFilter==='all'||button.dataset.categoryFilter==='all'));renderMap();selectShop(index);mapDetail?.scrollIntoView({behavior:'smooth',block:'nearest'});}
...
let benefitDay=koreaToday();setInterval(()=>{const day=koreaToday();if(day!==benefitDay){benefitDay=day;renderMap();renderTodayDiscovery();}},60000);
