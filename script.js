...
function initMarketMap(){
 ...
 marketLeafletMap.setView(DORIMRI_MAP_CENTER,MARKET_DEFAULT_ZOOM,{animate:false});
...
const PUBLIC_PROFILE_URL='https://renzehysxirjilvdxacv.supabase.co/rest/v1/store_public_profiles?select=store_id,store_name,store_address,store_phone,hero_image_url,short_intro,featured_menu_name,featured_menu_price,today_benefit,benefit_until&is_published=eq.true';
...
async function loadPublicStoreProfiles(){const selected=activeShopIndex;try{const r=await fetch(PUBLIC_PROFILE_URL,{headers:{apikey:PUBLIC_PROFILE_KEY,Authorization:`Bearer ${PUBLIC_PROFILE_KEY}`}});if(!r.ok)throw new Error('profile_http');publicStoreProfiles=await r.json();renderMap();renderTodayDiscovery();renderSceneRecommendations();if(selected!==null&&(!benefitOnly||hasActiveBenefit(shops[selected])))selectShop(selected,{pan:false});}catch{publicStoreProfiles=[];renderMap();renderTodayDiscovery();renderSceneRecommendations();if(selected!==null&&!benefitOnly)selectShop(selected,{pan:false});}}
...
function selectShop(index,{pan=true}={}){
 ...
 const activeMarker=markerByIndex.get(index);if(pan&&activeMarker&&marketLeafletMap)marketLeafletMap.panTo(activeMarker.getLatLng(),{animate:true,duration:.25});
...
function renderMap(){
 ...
 if(list.length)selectShop(list[0].i,{pan:false});else if(mapDetail)mapDetail.innerHTML='<span class="map-detail-tag">검색 결과 없음</span><h3>조건을 바꿔보세요</h3><p>상가명이나 업종을 다시 입력해 주세요.</p>';
}
initMarketMap();
...
if(activeShopIndex!==null)selectShop(activeShopIndex,{pan:false});
