...
export async function onRequestGet({request,env}){
 ...
 const canonicalStores=new Set(aliases.map(row=>row.store_id).filter(Boolean)).size;return reply({...health,identity:{mode:'store_id',canonicalStores,aliases:aliases.length},privacy:{visitor_identifiers:false,aggregation:'canonical_store_day_event'},scope:'tenant:cheonggye'});
}
