(()=>{
  ...
  async function save(event){event.preventDefault();const form=event.currentTarget,data=new FormData(form),id=String(data.get('id')||'').trim();const payload={id,section:data.get('section'),kind:data.get('kind'),title:data.get('title'),url:data.get('url'),note:data.get('note'),body:data.get('body'),sort_order:Number(data.get('sort_order')||100),visible:data.get('visible')==='on',featured:data.get('featured')==='on'};status(id?'수정사항을 저장하고 있습니다.':'새 자료를 등록하고 있습니다.');try{const response=await fetch(route('/api/resources'),{method:id?'PUT':'POST',headers:headers(),body:JSON.stringify(payload)});const result=await response.json();if(!response.ok)throw new Error(result.error||'save_failed');form.hidden=true;await load()}catch(error){console.error(error);status(error.message==='title_required'||error.message==='id_title_required'?'제목을 확인해 주세요.':'저장하지 못했습니다.',true)}}
